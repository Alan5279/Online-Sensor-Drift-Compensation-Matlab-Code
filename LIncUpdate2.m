...
%   invalid.
%   T:= the target for original HT
%   betaS:= the source output weight matrix
%Output:
...
%Status: untested
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [beta,K]=LIncUpdate2(beta0,K0,C_T,C_Tu,h,t,HT,HTu,Case,T,betaS)
    %Judge the case
    if(Case==1)
...
%         %The following are modified version
        ...
        TempPI=pinv(TempP);
        TempQ=TempHT*HTu';
        Temp=C_T*TempP+C_Tu*TempPI*(TempQ*TempQ');
        K=pinv(eye(size(Temp))+Temp);
        TempT=[T;t];
        beta=TempHT'*K*(C_T*TempT+C_Tu*TempPI*TempQ*HTu*betaS);
%         
