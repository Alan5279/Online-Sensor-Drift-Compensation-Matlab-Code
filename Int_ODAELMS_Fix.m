%This manuscript is used for IntegratedExp_FixCircle
%Status:tested
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:9   %For testing, only using the first set of training and testing
    ...
    for j=1:length(TestSet)
        ...
        if(Flag==Fix)%Labeling required
            ...
            if(y==0)
                ...
                [model,USet]=kenstone(TempSet,Selected);
                IncSet=setdiff(model,LSet);
                LSet=[LSet,IncSet];
                ...
                %target ELM has already been initialized
                TempSet=TestSet(1:j,2:endsize);
                [model,USet]=kenstone(TempSet,Selected);
                IncSet=setdiff(model,LSet);
                LSet=[LSet,IncSet];
    ...
    ErrorSubFolder=['ODAELMS-Fix-Error-B',num2str(i)];
    LNumSubFolder=['ODAELMS-Fix-LNum-B',num2str(i)];
    TimeSubFolder=['ODAELMS-Fix-Time-B',num2str(i)];
