        ...
        if (rand(1,1)>TempP||Error<0.1)%No KS selection process
            ...
            if (x<=2)%There is only one or two samples so far
                ...
                if (y==0)
                    ...
                    HT=h;
                    [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy,TY,beta] = ELM_K(TrainingData,Train_Target, TestingData,Test_Target, Elm_Type, Regularization_coefficient, Kernel_type, Kernel_para)
                else
